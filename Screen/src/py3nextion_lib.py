...
def nx_setBackground(ser, pageID, componentID, color):  # writes the color in the background of a component
    color_str = 'p[' + str(pageID) + '].b[' + str(componentID)+']''.bco=' + str(color)  # test here the "'"
    ser.write((color_str+EndCom).encode('latin-1'))
    return None

def nx_setGlobalVariable(ser, var, value):  # writes the value in the global variable
    value_str = var + '=' + str(value)
    ser.write((value_str+EndCom).encode('latin-1'))
    return None
